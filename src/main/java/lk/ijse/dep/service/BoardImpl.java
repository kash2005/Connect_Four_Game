        ...
        for (int i = 0; i < NUM_OF_COLS; i++) {
            for (int j = 0; j < NUM_OF_ROWS; j++) {
                if (pieces[i][1] == Piece.BLUE) {
                    if (((pieces[i][1] == pieces[i][2]) && (pieces[i][2] == pieces[i][3]) && pieces[i][1] == pieces[i][0])) {
                ...
                if (pieces[i][1] == Piece.GREEN) {
                    if (((pieces[i][1] == pieces[i][2]) && (pieces[i][2] == pieces[i][3]) && pieces[i][0] == pieces[i][1])) {
                        return new Winner(Piece.GREEN, i, 0, i, 1);
                    } else if (((pieces[i][1] == pieces[i][2]) && (pieces[i][2] == pieces[i][3]) && pieces[i][3] == pieces[i][4])) {
                        return new Winner(Piece.GREEN, i, 1, i, 4);
                    }
                }
                if (pieces[2][j] == Piece.BLUE) {
                    if (((pieces[3][j] == pieces[2][j]) && (pieces[2][j] == pieces[1][j]) && pieces[0][j] == pieces[1][j])) {
                        return new Winner(Piece.BLUE, 0, j, 3, j);
                    } else if ((pieces[2][j] == pieces[3][j] )&& (pieces[2][j] == pieces[1][j])&&(pieces[3][j]==pieces[4][j])){
                ...
                if (pieces[2][j] == Piece.GREEN){
                    ...
                    } else if ((pieces[3][j] == pieces[2][j] )&& (pieces[2][j] == pieces[1][j])&&(pieces[3][j]==pieces[4][j])){
